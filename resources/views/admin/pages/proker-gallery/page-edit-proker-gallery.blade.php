...
<div>
    <div class="card mb-8">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Edit Proker Galeri</h1>
        </div>
        <div class="card-body">
            <form method="POST" action="{{url('/proker-gallery/update',$getDetailProkerGallery->id)}}" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="mt-3">
                    ...
                    @foreach ($getAllProker as $itemProker)
                        <option value="{{$itemProker->id}}" @if ($itemProker->id == $getDetailProkerGallery->proker_id) selected @endif>{{$itemProker->alt_name}} - {{$itemProker->name}}</option>
                    @endforeach
                    </select>
                    @error('proker')
                        ...
                        <div class='flex items-center justify-center w-full mt-2'>
                            <label class='flex flex-col border-4 border-dashed w-full h-auto border-teal-500 group bg-gray-300'>
                                    <div class='flex flex-col items-center justify-center py-1'>
                                        <img id="preview" src="{{asset('assets/upload/proker-gallery')}}/{{$getDetailProkerGallery->image}}" alt="preview" class="object-cover h-32">
                                    </div>
                            </label>
                        </div>
                    </div>
                ...
                <div class="mt-3">
                    ...
                    <input type="text" name="caption" class="form-input w-full block rounded mt-1 p-3 border-2 @error('caption') border-red-500 @enderror focus:outline-none focus:border-teal-500" placeholder="Lorem ipsum dolor sit amet" value="{{$getDetailProkerGallery->caption}}">
                    @error('caption')
                    <span class="pl-1 text-xs text-red-600 text-bold">
                        {{$message}}
                    </span>
