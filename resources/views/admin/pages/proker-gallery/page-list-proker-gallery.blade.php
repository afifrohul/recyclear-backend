...
<div>
    <div class="card mb-8">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Tambah Proker Galeri</h1>
        </div>
        <div class="card-body">
            <form method="POST" action="{{url('proker-gallery/new')}}" enctype="multipart/form-data">
                @csrf
                <div class="mt-3">
                ...
                <div class="mt-3">
                    ...
                    <input type="text" name="caption" class="form-input w-full block rounded mt-1 p-3 border-2 @error('caption') border-red-500 @enderror focus:outline-none focus:border-teal-500" placeholder="Lorem ipsum dolor sit amet" value="{{old('caption')}}">
                    @error('caption')
                    <span class="pl-1 text-xs text-red-600 text-bold">
                        {{$message}}
                    </span>
    ...
    <div class="card">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">List Galeri Proker</h1>
        </div>
        <div class="card-body">
            <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg p-3">
                <table class="min-w-full divide-y divide-gray-200" id="dataTable">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($getAllProkerGallery as $item)  
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm text-gray-900">
                                    <div class="text-sm text-gray-900">
                                        <img class="h-16 object-cover m-auto" src="{{asset('assets/upload/proker-gallery')}}/{{$item->image}}">
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <form action="{{url('/proker-gallery/edit',$item->id)}}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-teal-500 h-10 w-10 rounded hover:bg-teal-600"><i class="fa fa-pencil text-white"></i></button>
                                </form>&nbsp;
                                <form action="{{url('/proker-gallery/destroy',$item->id)}}" method="POST" class="inline">
                                    @method('delete')
                                    @csrf
