...
<div>
    <div class="card mb-8">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Edit Proker Timeline</h1>
        </div>
        <div class="card-body">
            <form method="POST" action="{{url('/proker-timeline/update',$getDetailProkerTimeline->id)}}" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="mt-3">
                    ...
                    @foreach ($getAllProker as $itemProker)
                        <option value="{{$itemProker->id}}" @if ($itemProker->id == $getDetailProkerTimeline->proker_id) selected             @endif>{{$itemProker->alt_name}} - {{$itemProker->name}}</option>
                    @endforeach
                    </select>
                    @error('proker')
                ...
                <div class="mt-3">
                    ...
                    <input type="text" name="name" class="form-input w-full block rounded mt-1 p-3 border-2 @error('name') border-red-500 @enderror focus:outline-none focus:border-teal-500" placeholder="IO" value="{{$getDetailProkerTimeline->name}}">
                    @error('name')
                    <span class="pl-1 text-xs text-red-600 text-bold">
                        {{$message}}
                    </span>
...
<script>
    ...
    function app() {
      return {
        ...
        datepickerValue: '',
        selectedDate: "{{$getDetailProkerTimeline->date}}",
