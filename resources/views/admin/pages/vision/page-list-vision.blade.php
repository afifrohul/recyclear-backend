...
<div>
    <div class="card mb-8">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Tambah Visi</h1>
        </div>
        <div class="card-body">
            <form method="POST" action="{{url('vision/new')}}" enctype="multipart/form-data">
                @csrf
                <div class="mt-3">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <form action="{{url('/vision/edit',$item->id)}}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-teal-500 h-10 w-10 rounded hover:bg-teal-600"><i class="fa fa-pencil text-white"></i></button>
                                </form>
                                <form action="{{url('/vision/destroy',$item->id)}}" method="POST" class="inline">
                                    @method('delete')
                                    @csrf
