...
<div>
    <div class="card mb-8">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Tambah Post Galeri</h1>
        </div>
        <div class="card-body">
            <form method="POST" action="{{url('post-gallery/new')}}" enctype="multipart/form-data">
                @csrf
                <div class="mt-3">
                    <label class="text-gray-700 ml-1">Post : </label>
                    <select name="post" class="form-input mt-1 p-3 border-2 @error('post') border-red-500 @enderror focus:outline-none focus:border-teal-500 form-select appearance-none block w-full px-3
                    ...
                    @foreach ($getAllPost as $itemPost)
                        <option value="{{$itemPost->id}}">{{$itemPost->title}}</option>
                    @endforeach
                    </select>
                    @error('post')
                        ...
                        <div class='flex items-center justify-center w-full mt-2'>
                            <label class='flex flex-col border-4 border-dashed w-full h-auto border-teal-500 group bg-gray-300'>
                                    <div class='flex flex-col items-center justify-center py-1'>
                                        <img id="preview" src="{{asset('assets/upload/post/default.png')}}" alt="preview" class="object-cover h-32">
                                    </div>
                            </label>
                        </div>
                    </div>
    ...
    <div class="card">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">List Galeri Post</h1>
        </div>
        <div class="card-body">
            <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg p-3">
                <table class="min-w-full divide-y divide-gray-200" id="dataTable">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($getAllPostGallery as $item)  
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm text-gray-900">{{$loop->iteration}}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm text-gray-900">{{$item->judul_post}}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm text-gray-900">
                                    <div class="text-sm text-gray-900">
                                        <img class="h-16 object-cover m-auto" src="{{asset('assets/upload/post-gallery')}}/{{$item->image}}">
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <form action="{{url('/post-gallery/edit',$item->id)}}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-teal-500 h-10 w-10 rounded hover:bg-teal-600"><i class="fa fa-pencil text-white"></i></button>
                                </form>&nbsp;
                                <form action="{{url('/post-gallery/destroy',$item->id)}}" method="POST" class="inline">
                                    @method('delete')
                                    @csrf
